...
type TCreateFormValues = z.infer<typeof CreateFormSchema>;

// This can come from database or API
const defaultValues: Partial<TCreateFormValues> = {
  title: "",
};
...
const CreateForm = ({ username }: ICreateFormProps) => {
  ...
  const form = useForm<TCreateFormValues>({
  ...
  function onSubmit(data: TCreateFormValues) {
    mutate({
      title: data?.title,
      description: "",
      ...
      .then((gigId: Id<"gigs">) => {
        toast.info("Gig creates successfully");
        // form.setValue("title", "");
